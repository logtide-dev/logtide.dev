...
- **Official Ruby SDK** - the [`logtide` gem](https://rubygems.org/gems/logtide) for Ruby 3.1+, implementing the LogTide SDK spec v1.0 with leveled logging, structured exception capture, the Hub/Scope model, spans + OTLP export, W3C `traceparent` propagation, Rack middleware, a Rails Railtie, and a stdlib `Logger` bridge - standard library only, no runtime dependencies. Docs at [`/docs/sdks/ruby`](/docs/sdks/ruby/)
